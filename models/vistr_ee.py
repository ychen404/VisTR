"""
VisTR model and criterion classes with early exit
...
class VisTRWithEarlyExit(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_classes, num_frames, num_queries, aux_loss=False):
        ...
        #TODO pass num_decode_layers from arg
        self.num_decoder_layers = 6

        self.num_additional_layers = self.num_decoder_layers - 1
        # add 4 additional (since there are 5 decoder layers) class_emb and bbox_emb for early exit 
        self.class_embed_layers = nn.ModuleList(self.class_embed for _ in range(self.num_additional_layers)) 
        self.bbox_embed_layers = nn.ModuleList(self.bbox_embed for _ in range(self.num_additional_layers))
