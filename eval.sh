...
then
    ...
    do
        ...
        # taskset -p -c 0 $$ 
        echo "start $i times"
        CUDA_VISIBLE_DEVICES=1 python -m pdb inference_ee.py \
        --masks \
        --num_frames 3 \
        --num_queries 15 \
        --early_exit_layer $ee \
...
then
    ...
    # taskset -p -c 0 $$ 
    echo "start $i times"
        python inference_ee_mask_gen.py \
        --masks \
        --num_frames 3 \
        --num_queries 15 \
        --early_exit_layer $ee \
        --backbone resnet50 \
        --model_path ckpts/res50_test_segm_ee_3_fulldata_dist_num_frames_3_num_queries_15/checkpoint.pth \
        --img_path /home/users/yitao/Code/IFC/datasets/ytvis_2019/val/JPEGImages/ \
        --ann_path /home/users/yitao/Code/IFC/datasets/ytvis_2019/annotations/instances_val_sub.json \
        --save_path results/ee_"$ee"_segm_ee_fullds_dist_mask.json
        # --save_path results/ee_"$ee"_tinyds_results_from_scratch.json
        # --save_path results/ee_"$ee"_tinyds_results.json
        # --save_path results/ee_results.json >> inference_test_ee_"$ee".txt
        # --model_path ckpts/res50_test_segm_ee_"$ee"/checkpoint.pth \
